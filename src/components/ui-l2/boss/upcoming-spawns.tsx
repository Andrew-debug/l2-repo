...
export default function UpcomingSpawns() {
  ...
  // a single percentage shared by every row, not computed per-boss. Only
  // meaningful when a window is actually set — see the marker's own
  // conditional render below for the null (manual mode) case.
  const windowStartPct = globalRange
    ? (globalRange.minHours / globalRange.maxHours) * 100
    : 0;

  // getStatus/getKilledAt change identity every second (BossRespawnProvider's
  // live clock), and this recomputes on every render regardless — bailing
  // out while folded/closed skips it entirely when there's nothing on
  // screen to show for it (see the same guard in BossLevelNavigator).
  const isVisible = isOpen && !isFolded;

  const rows = !isVisible
    ? []
    : trackedBossIds
        .map((bossId) => {
          ...
          // No respawn window configured (globalRange null — "not set"/
          // manual mode, see BossRespawnProvider): status is always "dead"
          // and stays that way until marked alive by hand, so there's no
          // countdown to show, just how long ago it died — the row's own
          // render below switches to that text once minAt is null.
          if (!globalRange) {
            return {
              boss,
              status,
          ...
          const sortGroup =
            status === "alive" ? 0 : status === "pending" ? 1 : 2;
          const sortAt =
            status === "alive" ? maxAt : status === "pending" ? maxAt : minAt;

          return { boss, status, killedAt, minAt, maxAt, pct, sortGroup, sortAt };
        })
        .filter((r): r is NonNullable<typeof r> => r !== null)
        .sort((a, b) => a.sortGroup - b.sortGroup || a.sortAt - b.sortAt);

  // Outer box always reserves the full 320px-wide (w-80) footprint,
  // regardless of open/folded state — the same fix Map/Raid Bosses apply
  // to themselves (see level-navigator.tsx's own wrapper comment). This
  // sits in a column that's auto-sized to its own content once the
  // Up Next/Drop List/NPC Info group reaches its 2-column layout (see
  // main-content-row.tsx's @2xl grid, which drops the explicit column
  // template in favor of implicit auto-sized columns): without a
  // fixed-size outer box, folding down to the ~30px icon shrank that
  // column, sliding Drop List/NPC Info left into the space this panel
  // just vacated. Each DraggableWindow below is absolutely positioned
  // inside this box instead of contributing to its size.
  return (
    <div className="relative h-full min-h-0 w-80 shrink-0">
      {isFolded ? (
        ...
        <DraggableWindow
          id="up-next"
          className={cn(
            "absolute top-0 left-0 flex h-full min-h-0 w-80 flex-col",
            (!isOpen || !isOffsetHydrated) && "invisible pointer-events-none",
          )}
          initialOffset={offset}
          onOffsetChange={setOffset}
              ...
              <div className="flex h-full flex-col gap-1 p-2">
                ...
                {rows.length > 0 && (
                  <ul className="flex min-h-0 flex-1 flex-col gap-1 overflow-y-scroll custom-scrollbar pr-1">
                    {rows.map(({ boss, status, killedAt, minAt, maxAt, pct }) => (
                      <li key={boss.id}>
                        ...
                        >
                          <div className="relative flex items-center gap-1.5">
                            ...
                            {/* Inline rather than an absolute overlay: pending/dead
                                rows already have a duration span with its own
                                ml-auto, which pushes both it and these buttons to
                                the right edge as one group. alive rows have no
                                duration span, so the buttons carry ml-auto
                                themselves to get the same right alignment.
                                stopPropagation keeps clicks from also toggling
                                the row's selection. */}
                          </div>
                          <span
                            className={cn(
                              "absolute top-0.5 right-0.5 flex shrink-0 items-center gap-1",
                              status === "alive" && "ml-auto",
                            )}
                            onClick={(e) => e.stopPropagation()}
                            onKeyDown={(e) => e.stopPropagation()}
                          >
                            {/* Re-kills the boss right from here, resetting
                                  its tracked kill time to now. */}
                            <IconStateButton
                          ...
                          <div className="flex">
                            ...
                            >
                              {status === "alive" &&
                                `respawned ${formatDuration(now - (maxAt ?? now))} ago · Lv ${boss.level}`}
                              {status === "pending" &&
                                `respawn time started — guaranteed up in ${formatDuration((maxAt ?? now) - now)}`}
                              {status === "dead" &&
                                (minAt != null
                                  ? `respawn starts in ${formatDuration(minAt - now)} · Lv ${boss.level}`
                                  : `killed ${formatDuration(now - killedAt)} ago · Lv ${boss.level}`)}
                            </span>
                            {status === "pending" && (
                              <span className="ml-auto shrink-0 text-[13px] text-[#f5c518]">
                                {formatDuration((maxAt ?? now) - now)}
                              </span>
                            )}
                            {status === "dead" && minAt != null && (
